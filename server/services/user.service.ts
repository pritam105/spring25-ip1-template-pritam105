...
export const saveUser = async (user: User): Promise<UserResponse> => {
  ...
  try {
    ...
    return { _id: result.id, username: result.username, dateJoined: result.dateJoined };
  } catch (error) {
...
export const getUserByUsername = async (username: string): Promise<UserResponse> => {
  ...
  try {
    ...
    return { error: `Error occurred when getting user by username: ${error}` };
  }
};
...
export const loginUser = async (loginCredentials: UserCredentials): Promise<UserResponse> => {
  ...
  try {
    const user = await UserModel.findOne({ username: loginCredentials.username }).select(
      '-password',
    );
    if (!user) {
      throw Error('User not found');
    ...
    return { error: `Error occurred when logging in user: ${error}` };
  }
};
...
export const deleteUserByUsername = async (username: string): Promise<UserResponse> => {
  ...
  try {
    ...
    return { error: `Error occurred when deleting user by username: ${error}` };
  }
};
